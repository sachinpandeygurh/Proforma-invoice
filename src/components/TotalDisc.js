import { faCalendar, faCodeBranch, faDollar, faPenFancy, faPenNib, faSquarePlus, faTag } from '@fortawesome/free-solid-svg-icons';
...
const TotalDisc = () => {
  ...
  const getAmount=()=>{
  let amount=0;
  for(let i=0;i<data.length;i++)amount+=(data[i].rate*data[i].quantity);
  return amount;
  }
  useEffect(() => {
    getInvoice();
      ...
      <div className='col-md-4'>
        ...
        {includeGST && (
            ...
            <h2 className='d-flex justify-content-between border-top border-bottom py-4 px-2'>
              <span>Total(INR)</span> <span>&#x20b9; {Math.round(getAmount()+((getAmount()*18)/100))}</span>
            </h2>
